...
glm::vec3 Sky::sun_dir = glm::normalize(glm::vec3(1.f, 0.54f, 1.f));
...
Sky::Sky() {
  ...
  // scattering
  {
    ...
    auto group_size = getGroupSize(multiscattering_lut_size.x, multiscattering_lut_size.y);
    glDispatchCompute(group_size.x, group_size.y, 1);
    glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
  }
  ...
  // sky lut
  {
    sky_lut_shader_ = RenderUtils::create_shader(
      Options::instance()->getShaderPath("quad.vs"),
...
void Sky::generate_sky_lut(const Renderer& renderer) const {
  ...
  params.sun_radiance = glm::vec3(0.5f, 0.5f, 1.f);
